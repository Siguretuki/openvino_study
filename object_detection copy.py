...
def write_output_video(output_path, frames, frame_rate):
...
class VideoPlayer:
    def __init__(self, source, flip=False, skip_first_frames=0):
        self.video = cv2.VideoCapture(source)
        self.flip = flip
        self.fps = self.video.get(cv2.CAP_PROP_FPS)

def run_object_detection_and_save(source=0, flip=False, output_path=None, skip_first_frames=0):
    player = None
    output_frames = []
    try:
        ...
        def update_frame():
            nonlocal processing_times, output_frames
        ...
        if output_path is not None:
            write_output_video(output_path, output_frames, player.fps)
...
run_object_detection_and_save(source=video_file, flip=False, output_path=output_video_path)
