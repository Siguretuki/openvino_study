...
def run_object_detection_tkinter(source=0, flip=False, use_popup=False, skip_first_frames=0, output_video_path=None):
    ...
    results_list = []  # List to store the results for each frame
    ...
    try:
        ...
        def update_frame():
            ...
            results_list.append((frame.copy(), boxes))  # Store results for each frame
        ...
        # Process all frames' results and save as a video
        if output_video_path and results_list:
            video_writer = None
            try:
                for frame, boxes in results_list:
